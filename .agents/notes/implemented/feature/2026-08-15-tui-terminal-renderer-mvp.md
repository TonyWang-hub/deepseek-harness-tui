...
Known deferred work carried into a later cut: `read`/`search`/`web` tool result cards stay on the generic fallback; resume/backfill and the `/history` pager are unbuilt (nodes present at mount are the committed baseline, never replayed); only the first sub-question of a multi-part `ask_user_question` is answerable, and a question with no `options` (free text) shows an informational line rather than accepting input; Shift+Enter is a literal `\n` byte (Ctrl+J) stand-in outside the kitty keyboard protocol; `packages/bundle/tui-app`'s shipped `dsh --profile tui` composition is D2.3's job — this cut's own dev-run driver (`packages/tui/runtime/tests/dev-run.manual.ts`) is the only way to run it by hand today.
