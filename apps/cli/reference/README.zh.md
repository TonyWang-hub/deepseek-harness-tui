...
组合包名称先从 dsh 安装目录解析，再从 profile 目录解析。因此，内置组合包（`@deepseek-ai/dsh-base`、`@deepseek-ai/dsh-web-app`、`@deepseek-ai/dsh-headless`、`@deepseek-ai/dsh-tui-app`）始终来自当前运行的 `dsh` 所属的安装；树外组合包则来自 profile 中由 pnpm 管理的 `node_modules`。patch 行中的裸插件 `name` 会从 profile 目录开始，按照 Node 的模块解析规则逐级向父目录查找，直至由 dsh 维护的安装后备目录 `$DSH_HOME/profiles/node_modules`。该目录为 dsh 安装中的应用和组合包所依赖的每个包各维护一个符号链接，并在每次启动时修复这些链接。

`web`、`headless` 和 `tui` profile 首次使用时会从随附模板自动初始化（`web`：base + web-app；`headless`：base + headless；`tui`：base + tui-app）。其他缺失的 profile 会显式报错，并提示运行 `dsh plugin --profile <name> add <package>`。
...
`dsh --profile tui` 运行终端应用：第二个进程内 Client cordis `Context` 通过进程内连接触达 Web GUI 所用的同一套对象层；真实 TTY `stdout` 会在其上挂载 Ink 渲染器——提示词、流式输出、工具调用审批和提问都会渲染在终端里，渲染器退出（正常退出或 Ctrl-C）后进程随之退出。`--resume <sessionId>` 会打开一个既有会话而不是新建一个，但不会把它之前的记录回放进 scrollback。非 TTY 调用（管道、CI、没有终端接入）会响亮失败而不是挂起，因为组合中的其他部分不会产生任何输出或退出。随附 tui profile 挂载 Connection 的 Host 半区与 API 网关，但不挂载 HTTP 服务器、Web 运行时或浏览器客户端，因此同样不会打开监听端口。
