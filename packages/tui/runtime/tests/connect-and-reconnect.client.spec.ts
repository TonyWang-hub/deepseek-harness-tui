 ...
 * Typert Remote call, and reconnect-on-withdrawal — the landing-order slice
...
describe('tui-runtime: dual-context boot, ready handshake, Typert Remote, reconnect', () => {
  ...
  it('reconnects after the ApiProxy fiber withdraws, and a new call succeeds once it recomposes', async () => {
    const states: string[] = []
    const off = connection.hostDescription.subscribe(() => {
      states.push(connection.hostDescription.getSnapshot() === undefined ? 'reconnecting' : 'connected')
    })
    try {
